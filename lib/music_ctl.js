    ...
    return Widget.Box({
        ...
        children: [
            Widget.Label({
                ...
                connections: [[Mpris, self => {
                    const { trackArtists, trackTitle } = Mpris.players[0];
                    self.label = `${trackArtists.join(', ')} - ${trackTitle}`
                }]]
            }),
            Widget.Label({
                xalign: 0,
                ...
                connections: [[Mpris, self => {
                    self.label = `Current player: ${truncatePlayerName(Mpris.players[0].busName)}`
                }]]
            })
        ],
    })
    ...
    return Widget.Box({
        ...
        children: [
            ...
            Widget.ProgressBar({
                vertical: false,
                connections: [[Mpris, self => {
                    print(Mpris.players[0].position)
                    self.fraction = Mpris.players[0]?.position / Mpris.players[0]?.length
                }]]
            })
        ]
    })
...
export const MusicCtl = () => Widget.Window({
    name: 'music-ctl',
    anchor: ['top'],
